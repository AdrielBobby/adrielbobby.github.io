...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.matchMedia("(min-width: 769px)").matches) {
    ...
    // Hover effects
    const interactables = document.querySelectorAll('a, button, .card, .email-pill, .social-icons a, .hero-ascii-container, .scan-card, .secure-console, .contact-item, .reset-link');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let state = 'idle'; // idle, connecting, encrypted, connected
  let idleInterval;
  ...
  if (consoleEl) {
    ...
    if (resetLink) {
      resetLink.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation(); // Prevent console click
        resetUplink();
      });
    }
  }
  ...
  function startIdleAnim() {
    ...
    idleInterval = setInterval(() => {
      ...
      s1.textContent = sequence[frame][0];
      s2.textContent = sequence[frame][1];
      s3.textContent = sequence[frame][2];
      s4.textContent = sequence[frame][3];
      s5.textContent = sequence[frame][4];
    }, 600);
  }
  ...
  function initiateUplink() {
    ...
    addLog("Initiating uplink handshake...", 0);
    addLog("Sending request to SAT-AB01...", 600);
    addLog("Awaiting response...", 1200);

    setTimeout(() => {
      addLog("Uplink Established.", 0, 'var(--primary)');
      addLog("Negotiating encryption key...", 600);
      setTimeout(() => {
        startDecryption();
      }, 1500);
    }, 2500);
  ...
  function addLog(text, delay, color = null) {
    setTimeout(() => {
  ...
  function showPayload() {
    ...
    // Use updated animation class name
    consoleEl.classList.remove('pulse-success');
    void consoleEl.offsetWidth;
    consoleEl.classList.add('pulse-success');
  }
  ...
  function resetUplink() {
    ...
    statusLines.innerHTML = `
            <div class="line">LINK STATUS: <span class="status-idle">IDLE</span></div>
            <div class="line action-hint" id="actionHint">> CLICK TO INITIATE UPLINK <span class="cursor-blink">_</span></div>
        `;
    startIdleAnim();
  }
});
